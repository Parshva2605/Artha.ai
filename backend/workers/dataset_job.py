...
from backend.pipeline.synthesizer import synthesize_rows_for_language
...
def generate_dataset_task(job_id: str, request: dict):
    ...
    use_synthetic = bool(request_payload.get("use_synthetic", False))
    ...
    try:
        ...
        if total_scraped_rows == 0 and not use_synthetic:
            raise RuntimeError("No data collected")
        if total_scraped_rows == 0 and use_synthetic:
            logger.info("No scraped rows found; proceeding with synthetic generation because use_synthetic=True")
        ...
        for lang_code in languages:
            ...
            # If synthetic data is enabled and this language is short, synthesize additional rows
            if use_synthetic and len(lang_balanced) < quantity_per_language:
                needed = max(0, quantity_per_language - len(lang_balanced))
                synth = synthesize_rows_for_language(
                    language_config=get_config_by_code(lang_code),
                    existing_rows=lang_balanced,
                    label_type=label_type,
                    target_count=needed,
                    custom_labels=custom_labels,
                )
                synthetic_rows_total += len(synth)
                lang_balanced.extend(synth)
        ...
        result_summary = {
            **_quality_report_to_dict(quality_report),
            "custom_labels": custom_labels,
            "scraped_rows": int(total_scraped_rows),
            "synthetic_rows": int(synthetic_rows_total),
